 ...
 *
 * Conversion of libavcode/avcodec.h
...
const
  ...
  *)
  ...
  CODEC_FLAG_TRUNCATED       = $00010000; (** Input bitstream might be truncated at a random location instead of only at frame boundaries. *)
...
(* DEPRECATED, directly use the AV_PICTURE_TYPE_* enum values *)
  FF_I_TYPE  = AV_PICTURE_TYPE_I; ///< Intra
  FF_P_TYPE  = AV_PICTURE_TYPE_P; ///< Predicted
  FF_B_TYPE  = AV_PICTURE_TYPE_B; ///< Bi-dir predicted
  FF_S_TYPE  = AV_PICTURE_TYPE_S; ///< S(GMC)-VOP MPEG4
  FF_SI_TYPE = AV_PICTURE_TYPE_SI; ///< Switching Intra
  FF_SP_TYPE = AV_PICTURE_TYPE_SP; ///< Switching Predicted
  FF_BI_TYPE = AV_PICTURE_TYPE_BI;
{$IFEND}
 ...
 *)
    duration:     cint;
    destruct:     procedure (packet: PAVPacket); cdecl;
    priv:         pointer;
...
{$IFEND}
  ...
  TAVCodecContext = record {720}
    ...
    draw_horiz_band: procedure (s: PAVCodecContext;
                                const src: PAVFrame; offset: PArray4Integer;
   ...
   qmax: cint;
     ...
     * - decoding: Set by user.
     *)
    realloc: function (s: PAVCodecContext; buf: pByteArray; buf_size: cint): pByteArray; cdecl;
    {$IFEND}
     ...
     * - decoding: Set by user, will be converted to uppercase by libavcodec during init.
     *)
    stream_codec_tag: cuint; //array [0..3] of AnsiChar;
...
{$ENDIF}
     ...
     * - decoding: Set by libavcodec
     *)
    color_transfer: TAVColorTransferCharacteristic;
 ...
 *)
  TAVCodec = record
    ...
    max_lowres: cuint8;                          ///< array of support channel layouts, or NULL if unknown. array is terminated by 0
     ...
     *)
    defaults: {const} PAVCodecDefault;
...
procedure av_destruct_packet_nofree(pkt: PAVPacket);
  cdecl; external LIB_AVCODEC; deprecated 'use NULL instead';
...
function audio_resample (s: PReSampleContext; output: pcint16; input: pcint16; nb_samples: cint): cint;
  cdecl; external LIB_AVCODEC;
 ...
 *)
function av_resample (c: PAVResampleContext; dst: pcint16; src: pcint16; var consumed: cint;
...
function avcodec_get_pix_fmt_name(pix_fmt: TAVPixelFormat): PAnsiChar;
  cdecl; external LIB_AVCODEC; deprecated 'Deprecated in favor of av_get_pix_fmt_name()';
{$IFEND}
 ...
 *)
function avcodec_version(): cunsigned;
  cdecl; external LIB_AVCODEC;
...
procedure avcodec_init();
  cdecl; external LIB_AVCODEC; deprecated 'his function is called automatically from avcodec_register() and avcodec_register_all(), there is no need to call it manually' ;
{$IFEND}
 ...
 *)
function av_get_profile_name(const codec: PAVCodec; profile: cint): {const} PAnsiChar;
  cdecl; external LIB_AVCODEC;
...
procedure avcodec_get_context_defaults(s: PAVCodecContext);
  cdecl; external LIB_AVCODEC; deprecated;
...
function avcodec_alloc_context(): PAVCodecContext;
  cdecl; external LIB_AVCODEC; deprecated 'use avcodec_alloc_context3()';

(** THIS FUNCTION IS NOT YET PART OF THE PUBLIC API!
 *  we WILL change its arguments and name a few times! *)
function avcodec_alloc_context2(MediaType: TAVMediaType): PAVCodecContext;
  cdecl; external LIB_AVCODEC; deprecated 'use avcodec_alloc_context3()';
{$IFEND}
 ...
 *)
function avcodec_copy_context(dest: PAVCodecContext; const src: PAVCodecContext): cint;
  cdecl; external LIB_AVCODEC;
...
function avcodec_thread_init(s: PAVCodecContext; thread_count: cint): cint;
  cdecl; external LIB_AVCODEC; deprecated 'Set s->thread_count before calling avcodec_open2() instead of calling this';
{$IFEND}
...
function avcodec_open(avctx: PAVCodecContext; codec: PAVCodec): cint;
  cdecl; external LIB_AVCODEC; deprecated 'use avcodec_open2';
{$IFEND}
...
function avcodec_parse_frame(avctx: PAVCodecContext; var pdata: pByteArray;
                        data_size_ptr: cint;
 ...
 *)
function avcodec_encode_audio(avctx: PAVCodecContext; buf: pcuint8;
                      buf_size: cint; const samples: PCint16): cint;
  cdecl; external LIB_AVCODEC;
 ...
 *)
function avcodec_encode_video(avctx: PAVCodecContext; buf: pByteArray;
                      buf_size: cint; const pict: PAVFrame): cint;
  cdecl; external LIB_AVCODEC;
function avcodec_encode_subtitle(avctx: PAVCodecContext; buf: pByteArray;
                      buf_size: cint; const sub: PAVSubtitle): cint;
  cdecl; external LIB_AVCODEC;
...
function av_get_pict_type_char(pict_type: cint): AnsiChar;
  cdecl; external LIB_AVCODEC; deprecated 'Use av_get_picture_type_char() instead.';
{$IFEND}
...
function av_get_bits_per_sample_format(sample_fmt: TAVSampleFormat): cint;
  cdecl; external LIB_AVCODEC; deprecated 'Use av_get_bits_per_sample_fmt() instead.';
{$IFEND}
...
type
  ...
  TAVCodecParser = record
    ...
    parser_parse: function(s: PAVCodecParserContext;
                           ...
                           var poutbuf: PByteArray; poutbuf_size: PCint;
...
type
  ...
  TAVBitStreamFilter = record
    ...
    init: function(bsfc: PAVBitStreamFilterContext; args: PAnsiChar): cint;  cdecl;
  end;
 ...
 *)
function av_picture_crop(dst: PAVPicture; src:  PAVPicture;
 ...
 * Pad image.
 *)
function av_picture_pad(dst: PAVPicture; src: PAVPicture; height: cint; width: cint; pix_fmt: TAVPixelFormat;
...
type
  TlockmgrFunc = function(mutex: Ppointer; op: TAVLockOp):cint;

function av_lockmgr_register(cb: TlockmgrFunc): cint;
  cdecl; external LIB_AVCODEC;
